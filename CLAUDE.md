...
| `reflow.py` | Phase 2 helper — join soft-wrapped prose lines into one line per paragraph and strip emphasis that wraps a whole paragraph; leaves headings, lists, tables, blockquotes, fenced code, and table-of-contents/index entries (a line ending in an arabic or roman page reference) untouched; also splits a run-on contents line the model collapsed into one line back into one entry per line |
...
| `fences.py` | Shared helper — CommonMark fenced-code-block rules (`FENCE_RE`, `closes_fence`, `next_fence_state`) used by `headings` and `reflow` to leave code blocks untouched |
| `pdf_errors.py` | Shared `PDF_ERRORS` exception tuple (covers PyMuPDF `RuntimeError`-family and `pymupdf.mupdf.FzErrorBase`-family errors plus I/O) for the per-page extraction guards, plus the `open_guarded` context manager that opens a PDF inside the guard and always closes it (swallowing close-time `PDF_ERRORS`) so cleanup can't break a helper's "never fail a page" guarantee |
